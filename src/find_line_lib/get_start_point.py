...
def get_start_point(
    ...
    Args:
        ...
        start_point: 起始扫描的坐标点 (x, y)，若为None则根据direction默认
            - horizontal模式：默认从图像底部中心 (w//2, h-1) 开始
            - vertical模式：默认从图像左部中间 (0, h//2) 开始
    ...
    if direction == "horizontal":
        ...
        init_col = center_col if start_point is None else start_point[0]
        init_row = h - 1 if start_point is None else start_point[1]
        ...
        for row in range(init_row, row_min - 1, -1):
            ...
            for col in range(init_col, col_min, -1):
                if bin_img[row, col] == 255 and bin_img[row, col - 1] == 0:
                    left_point = (col, row)
                    break
            else:
                # 情况B（补丁）：如果循环正常走完没突破，且中心点是白的，说明从中心到左边界全白
                if bin_img[row, init_col] == 255:
                    left_point = (col_min, row)
        ...
        # 垂直模式：从 start_point 开始向右逐列垂直扫描
        init_col = 0 if start_point is None else start_point[0]
        init_row = h // 2 if start_point is None else start_point[1]
        ...
        for col in range(init_col, col_max):
            ...
            # 向上扫描，找上边界
            for row in range(init_row, row_min - 1, -1):
                if bin_img[row, col] == 255 and bin_img[row - 1, col] == 0:
                    upper_point = (col, row)
                    break
            else:
                if bin_img[init_row, init_col] == 255:
                    upper_point = (col, row_min)
